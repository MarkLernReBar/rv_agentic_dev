...
def _build_prompt(company: Dict[str, Any], run: Dict[str, Any], needed: int) -> str:
    ...
    prompt = (
        ...
        "Skip anyone who would obviously be outside RentVine's ICP. Use MCP tools for verified emails and LinkedIn URLs.\n"
    )
    return prompt
    ...
    try:
        while True:
            ...
            if max_loops is not None and loops >= max_loops:
                ...
                # Before exiting, if we're focused on a single run and it still
                # has a contact gap, mark it as needing user decision and send
                # a notification with next-best options.
                if run_filter_id:
                    gap = supabase_client.get_contact_gap_summary(run_filter_id)
                    total_gap = int(gap.get("contacts_min_gap_total") or 0) if gap else 0
                    if total_gap > 0:
                        run = supabase_client.get_pm_run(run_filter_id) or {}
                        criteria = run.get("criteria") or {}
